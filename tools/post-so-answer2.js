...
async function postAnswer() {
  try {
    // Find the answer textarea and fill it (using new Stacks editor ID)
    console.log('Finding answer textarea...');
    await evalInTab(wsUrl, `
      const textarea = document.getElementById('js-stacks-editor-content') || document.querySelector('textarea[name="post-text"]') || document.querySelector('.js-stacks-editor-backing-textarea');
      if (textarea) {
    ...
    await evalInTab(wsUrl, `
      const btn = document.getElementById('submit-button') || document.querySelector('button[id*="submit"]') || Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('Post Your Answer'));
      if (btn) {
        btn.click();
    ...
    const result = await evalInTab(wsUrl, 'document.body.innerText');
    const pageText = result && result.value ? result.value : JSON.stringify(result);
