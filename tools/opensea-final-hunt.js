...
async function main() {
  ...
  try {
    ...
    if (await makeOfferBtn.count() > 0) {
      ...
      // Try to fill the offer form - find all inputs in the modal
  ...
  // Try another item page to see if different items trigger different mutations
  console.log('\n[+] Visiting another item page...');
  try {
    await page.goto('https://opensea.io/item/ethereum/0x8a90cab2b38dba80c64b7734e58ee1db38b8992e/1', { waitUntil: 'networkidle', timeout: 60000 });
    await page.waitForTimeout(3000);
    ...
    if (await makeOfferBtn.count() > 0) {
      await makeOfferBtn.click();
      await page.waitForTimeout(3000);
      console.log('  Clicked Make Offer on second item');
      ...
      // Select expiration
      const expirationSelect = page.locator('select[name="expiration"], select').first();
      ...
      // Try submit
      const submitBtn = page.locator('button:has-text("Make offer"):not([disabled]), button:has-text("Submit"):not([disabled]), button[type="submit"]:not([disabled])').first();
      if (await submitBtn.count() > 0) {
        await submitBtn.click();
        await page.waitForTimeout(5000);
        console.log('  Clicked submit on second item');
      }
    ...
    console.log('  Second item error:', e.message);
  }
