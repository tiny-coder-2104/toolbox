...
async function main() {
  ...
  // Also listen for responses to see if there are any errors
  client.on('Network.responseReceived', (params) => {
    if (params.response.url.includes('graphql') && params.response.status >= 400) {
      console.log('[GraphQL Error Response]', params.response.url, params.response.status);
    }
  });
  
  // Go to item page and try to actually submit an offer
  console.log('\n[+] Visiting item page to submit offer...');
  await page.goto('https://opensea.io/item/ethereum/0xbc4ca0eda7647a8ab7c2061c2e118a18a936f13d/1', { waitUntil: 'networkidle', timeout: 60000 });
  await page.waitForTimeout(3000);
  
  console.log('  Clicking "Make Offer" button...');
  try {
    ...
    if (await makeOfferBtn.count() > 0) {
      ...
      await page.waitForTimeout(3000);
      console.log('  Clicked Make Offer - modal should be open');
      
      // Take screenshot of modal
      const screenshotPath = path.join(SCREENSHOT_DIR, `${TIMESTAMP}-opensea-offer-modal.png`);
      await page.screenshot({ path: screenshotPath, fullPage: true });
      console.log(`  Screenshot: ${screenshotPath}`);
      
      // Try to fill the offer form
      console.log('  Looking for offer form inputs...');
      
      // Try different input selectors
      const amountInput = page.locator('input[name="amount"], input[placeholder*="amount" i], input[placeholder*="Amount" i], input[type="number"]').first();
      if (await amountInput.count() > 0) {
        await amountInput.fill('0.01');
        await page.waitForTimeout(1000);
        console.log('  Filled amount: 0.01');
      }
      
      // Look for expiration dropdown
      const expirationSelect = page.locator('select[name="expiration"], select[placeholder*="expiration" i]').first();
      if (await expirationSelect.count() > 0) {
        await expirationSelect.selectOption({ index: 1 });
        await page.waitForTimeout(500);
        console.log('  Selected expiration');
      }
      
      // Look for submit button in modal
      const submitBtn = page.locator('[data-testid="Overlay"] button:has-text("Make offer"), [data-testid="Overlay"] button:has-text("Submit"), [data-testid="Overlay"] button[type="submit"]').first();
      if (await submitBtn.count() > 0) {
        console.log('  Found submit button in modal, clicking...');
        await submitBtn.click();
        await page.waitForTimeout(5000); // Wait for mutation to fire
  ...
  // Try settings page - actually try to save changes
  console.log('\n[+] Visiting settings page to trigger profile update...');
  await page.goto('https://opensea.io/account/settings', { waitUntil: 'networkidle', timeout: 60000 });
  await page.waitForTimeout(3000);
  
  try {
    // Find editable fields and modify them
    const bioTextarea = page.locator('textarea[name="bio"], textarea[placeholder*="bio" i], textarea[placeholder*="Bio" i]').first();
    if (await bioTextarea.count() > 0) {
      const currentValue = await bioTextarea.inputValue();
      await bioTextarea.fill(currentValue + ' [test]');
      await page.waitForTimeout(1000);
      console.log('  Modified bio field');
    ...
    console.log('  Settings interaction error:', e.message);
  }
  
  // Try creating a listing (if we have items)
  console.log('\n[+] Visiting profile to check for listing creation...');
  await page.goto('https://opensea.io/tinycoderstudio', { waitUntil: 'networkidle', timeout: 60000 });
  await page.waitForTimeout(3000);
  
  try {
    // Look for "List" or "Sell" button on owned items
    const listBtn = page.locator('button:has-text("List"), button:has-text("Sell"), a:has-text("List"), a:has-text("Sell")').first();
    if (await listBtn.count() > 0) {
      await listBtn.click();
      await page.waitForTimeout(3000);
      console.log('  Clicked List/Sell button');
      
      // Try to fill listing form
      const priceInput = page.locator('input[name="price"], input[placeholder*="price" i], input[placeholder*="Price" i]').first();
      ...
      const submitBtn = page.locator('button:has-text("List"), button:has-text("Complete listing"), button[type="submit"]').first();
      if (await submitBtn.count() > 0) {
        await submitBtn.hover();
        await page.waitForTimeout(1000);
        console.log('  Hovered submit button');
      }
      
      await page.keyboard.press('Escape');
      await page.waitForTimeout(1000);
    } else {
      console.log('  No List/Sell button found (may not own items)');
    }
  } catch (e) {
    console.log('  Listing interaction error:', e.message);
  }
