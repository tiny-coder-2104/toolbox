...
async function main() {
  ...
  // Set up CDP network interception - capture ALL requests
  ...
  client.on('Network.requestWillBeSent', (params) => {
    ...
    if (request.method === 'POST') {
      console.log('[POST Request]', url, request.postData ? request.postData.substring(0, 200) : 'no body');
    }
  });
  ...
  try {
    ...
    if (await makeOfferBtn.count() > 0) {
      ...
      // Find amount input - try multiple selectors
      const amountSelectors = [
        'input[name="amount"]',
        'input[placeholder*="amount" i]',
        'input[placeholder*="Amount" i]',
        'input[type="number"]',
        '[data-testid="Overlay"] input[type="number"]',
        '[data-testid="Overlay"] input[name="amount"]'
      ];
      ...
      for (const selector of amountSelectors) {
        ...
        if (await input.count() > 0) {
          ...
          console.log(`  Filled amount: 0.01 using ${selector}`);
          break;
        }
      }
      ...
      for (const selector of expirationSelectors) {
        ...
        if (await select.count() > 0) {
          ...
          console.log(`  Selected expiration using ${selector}`);
          break;
        }
      }
      ...
      // Look for submit button in modal - try multiple selectors
      const submitSelectors = [
        '[data-testid="Overlay"] button:has-text("Make offer"):not([disabled])',
        '[data-testid="Overlay"] button:has-text("Submit"):not([disabled])',
        '[data-testid="Overlay"] button[type="submit"]:not([disabled])',
        '[data-testid="Overlay"] button:has-text("Make Offer"):not([disabled])',
        'button:has-text("Make offer"):not([disabled])',
      ...
      if (!clicked) {
        ...
        // Try to find any enabled button in modal
        const anyBtn = page.locator('[data-testid="Overlay"] button:not([disabled])').first();
        if (await anyBtn.count() > 0) {
          const text = await anyBtn.textContent();
          console.log(`  Found alternative button: ${text}, clicking...`);
          await anyBtn.click();
  ...
  try {
    await page.goto('https://opensea.io/account/settings', { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForTimeout(5000);
  ...
  const allPostRequests = allRequests.filter(e => e.method === 'POST');
  
  console.log(`[+] GraphQL POST requests: ${postRequests.length}`);
  console.log(`[+] All POST requests: ${allPostRequests.length}`);
  
  // Print all POST requests for analysis
  console.log('\n=== ALL POST REQUESTS ===');
  allPostRequests.forEach(r => {
  ...
  md += `\n## All POST Requests (Non-GraphQL)\n`;
  const nonGraphQLPosts = allPostRequests.filter(r => !r.url.includes('graphql'));
  if (nonGraphQLPosts.length > 0) {
    md += `| URL | Resource Type |\n`;
    md += `|---|---|\n`;
    nonGraphQLPosts.forEach(r => {
      md += `| ${r.url} | ${r.resourceType} |\n`;
    });
  } else {
    md += `None\n`;
  }
  ...
  console.log(`Total POST requests (all): ${allPostRequests.length}`);
