...
async function submitBugReport() {
  ...
  try {
    ...
    const isLoggedIn = await page.locator('text=New Report, text=Submit Report, [data-testid="new-report"]').first().isVisible().catch(() => false);
    ...
    if (!isLoggedIn) {
      ...
      // Fill login form
      await page.fill('input[type="email"], input[name="email"]', 'bugcrowd-slgzf9xw@bugbounty-test.example.com');
      await page.fill('input[type="password"], input[name="password"]', 'DsKnL6QXEi&sOF4T');
      await page.click('button[type="submit"], input[type="submit"], button:has-text("Log in")');
      await page.waitForLoadState('networkidle');
      
      await page.screenshot({ path: screenshotPath.replace('.png', '-02-after-login.png'), fullPage: true });
    }
    
    // Navigate back to OpenSea page if redirected
    if (!page.url().includes('opensea')) {
    ...
    const newReportBtn = page.locator('a:has-text("New Report"), button:has-text("New Report"), a:has-text("Submit Report"), button:has-text("Submit Report"), [href*="new"]').first();
    await newReportBtn.waitFor({ state: 'visible', timeout: 30000 });
    await newReportBtn.click();
    await page.waitForLoadState('networkidle');
    
    await page.screenshot({ path: screenshotPath.replace('.png', '-03-report-form.png'), fullPage: true });
    
    // Fill the report form
    console.log('Filling report form...');
    
    // Title
    await page.fill('input[name="title"], input[id*="title"], textarea[name="title"]', 'Unauthenticated IDOR: Hidden NFT Enumeration via ProfileItemsListQuery');
    ...
    // VRT Category - this might be a multi-select or dropdown
    // Try to find and select the VRT category
...
Affected endpoint: GET https://gql.opensea.io/graphql with persisted query hash c21d7fe7e333865ed58f5f4143c7ef7e9f684309e0dea7a8ec7eb938df2bb11a`;
    ...
    await page.fill('textarea[name="description"], textarea[id*="description"], textarea[name="details"]', description);
    
    await page.screenshot({ path: screenshotPath.replace('.png', '-04-filled-form.png'), fullPage: true });
