...
const KNOWN_HASHES = new Set([
  ...
  'a42c430444701592ba32f808bcb62a18a8224e8223f60bbaa270865fc4edff09', // WalletProfileStatsQuery (newly found)
]);

// Store all captures globally
let allCaptured = [];
...
async function main() {
  ...
  // Inject GraphQL capture script that persists across navigations
  console.log('[+] Injecting persistent GraphQL capture script...');
  await context.addInitScript(() => {
    // Use a global that persists across navigations
    if (!window.__openseaGraphQLCaptures) {
      window.__openseaGraphQLCaptures = [];
    }
    window._capturedGraphQL = window.__openseaGraphQLCaptures;
    ...
    window.fetch = async function(...args) {
      ...
      if (typeof url === 'string' && url.includes('graphql')) {
        ...
        console.log('[GraphQL Captured]', JSON.stringify(entry));
      }
  ...
  page.on('console', msg => {
    const text = msg.text();
    if (text.includes('[GraphQL Captured]')) {
  ...
  async function getCaptures() {
    return await page.evaluate(() => window.__openseaGraphQLCaptures || []);
  }
  ...
  async function visitAndCapture(url, name, interactFn = null) {
    ...
    try {
      ...
      const captured = await getCaptures();
      console.log(`  Total captured so far: ${captured.length}`);
  ...
  await visitAndCapture('https://opensea.io/item/ethereum/0xbc4ca0eda7647a8ab7c2061c2e118a18a936f13d/1', 'Item page - interact', async (page) => {
    ...
    try {
      ...
      if (await makeOfferBtn.count() > 0) {
        ...
        // Try to fill offer form if modal opens
  ...
  // Try settings page interactions (profile update)
  await visitAndCapture('https://opensea.io/account/settings', 'Settings page - interact', async (page) => {
    ...
    try {
      // Look for save button or any interactive element
      const saveBtn = page.locator('button:has-text("Save"), button:has-text("Update"), button[type="submit"]').first();
      if (await saveBtn.count() > 0) {
        // Don't actually save, just hover to trigger any pre-fetch
        await saveBtn.hover();
        await page.waitForTimeout(1000);
        console.log('  Hovered save button');
      }
      
      // Check for any form inputs that might trigger mutations on blur
  ...
  await visitAndCapture('https://opensea.io/collection/boredapeyachtclub', 'Collection page - interact', async (page) => {
    ...
    try {
      // Click on an item to trigger detail view
  ...
  await visitAndCapture('https://opensea.io/tinycoderstudio', 'Profile page - interact', async (page) => {
    ...
    try {
      ...
      if (await editBtn.count() > 0) {
        ...
        // If modal/form opens, interact with it
  ...
  // Get final captures
  const finalCaptured = await getCaptures();
